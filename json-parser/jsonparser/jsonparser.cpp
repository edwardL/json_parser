...
JsonParser::JsonParser():m_json_root(0),m_cur_path_deepth()
{
...
bool JsonParser::Init(const char* str)
{
...
{
	json_object* cur_json_obj = m_cur_node_path[m_cur_path_deepth-1];
...
{
	if(m_cur_path_deepth >= MAX_PATH_DEEPTH) return false;
	...
	m_cur_node_path[m_cur_path_deepth++] = next_json_obj;
	return true;
}
...
bool JsonParser::Down(int index)
{
	...
	if(next_json_obj == 0) return false;
	if(json_object_get_type(next_json_obj) != json_type_object && json_object_get_type(next_json_obj) != json_type_array) return false;
	m_cur_node_path[m_cur_path_deepth-1];
	return true;
}

void JsonParser::Up()
{
	if(m_cur_path_deepth > 1) --m_cur_path_deepth;
}
...
{
	json_object* cur_json_oj = m_cur_node_path[m_cur_path_deepth-1];
	if(json_object_get_type(cur_json_oj) != json_type_object) return false;
	json_object_object_add(cur_json_oj,key, json_object_new_double(value));
	return true;
}
...
bool JsonParser::ObjSetObjAndDown(const char* key)
{
	...
	m_cur_node_path[m_cur_path_deepth++] = new_obj;
	return true;
}
...
bool JsonParser::ObjSetArrayAndDown(const char* key)
{
	...
	m_cur_node_path[m_cur_path_deepth++] = new_array;
	return true;
}
...
bool JsonParser::ArrayPushString(const char* value)
{
	json_object* cur_json_obj = m_cur_node_path[m_cur_path_deepth-1];
	if(json_object_get_type(cur_json_obj) != json_type_array) return false;
